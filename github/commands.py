...
from mautrix.types import Event, EventType, ReactionEvent, RelationType
            ...
            try:
                ...
                if error:
                    if e.type == "INSUFFICIENT_SCOPES":
                        await evt.reply(
                            "Your login doesn't have sufficient access to do that. "
...
class Commands:
    ...
    async def handle_message(
        self, evt: MessageEvent, client: GitHubClient, webhook_meta: Dict[str, Any]
    ) -> None:
        try:
        ...
        if full_action in commentable_actions:
            await client.mutate(
                query="addComment(input: $input) { clientMutationId }",
    ...
    async def handle_reaction(
        self, evt: ReactionEvent, client: GitHubClient, webhook_meta: Dict[str, Any]
    ) -> None:
        reaction_map = {
            "👍": "THUMBS_UP",
            "👎": "THUMBS_DOWN",
        ...
        if webhook_meta["event_type"] == "issues" and webhook_meta["action"] == "opened":
            ...
            return
        await client.mutate(
            query="addReaction(input: $input) { clientMutationId }",
    ...
    async def raw_query(self, evt: MessageEvent, query: str, client: GitHubClient) -> None:
        ...
        await evt.reply(
            "<pre><code class='language-json'>" f"{json.dumps(resp, indent=2)}" "</code></pre>",
            allow_html=True,
        )
    ...
    async def create_issue(
        self, evt: MessageEvent, repo: Tuple[str, str], data: str, client: GitHubClient
    ) -> None:
        ...
        if not repo:
            ...
            return
        repo_id = await client.query(
            query="repository (name: $name, owner: $owner) { id }",
            args="$owner: String!, $name: String!",
            variables={"owner": repo[0], "name": repo[1]},
            path="repository.id",
        )
        issue = await client.mutate(
            query="createIssue(input: $input) { issue { number url } }",
    ...
    async def webhook(
        self, evt: MessageEvent, repo: Tuple[str, str], client: GitHubClient
    ) -> None:
        pass
    ...
    async def webhook_list(self, evt: MessageEvent) -> None:
        ...
        info = "\n".join(
            f"* `{hook.repo}` added by " f"[{hook.user_id}](https://matrix.to/#/{hook.user_id})"
            for hook in hooks
        )
    ...
    async def webhook_create(
        ...
        if existing:
            ...
            return
        webhook = await self.bot.webhook_manager.create(repo_name, evt.sender, evt.room_id)
        try:
                ...
                try:
                    ...
                    if e.status_code == 404:
                        ...
                        self.bot.log.warning(
                            f"Failed to remove {webhook_info} from GitHub", exc_info=True
                        )
                else:
                    await evt.reply("Webhook deleted successfully")
                    return
